...
complete -c wpmusic -l refresh-cache -d "force refresh of local cache from MySQL server" -f
complete -c wpmusic -l no-cache -d "bypass local cache, always use MySQL server directly" -f
complete -c wpmusic -l test-db-connection -d "test database connection and exit" -f

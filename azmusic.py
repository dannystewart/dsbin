...
blob_service_client = BlobServiceClient.from_connection_string(connection_string).get_container_client(
    "music"
)
...
def validate_folder(folder):
    """
    Validates that the folder is one of the allowed folders.

    Args:
        folder (str): Folder name.
    """
    if folder not in ALLOWED_FOLDERS:
        raise ValueError(f"Folder must be one of the following: {', '.join(ALLOWED_FOLDERS)}")
...
def upload_to_azure(blob_name, temp_output_file):
    ...
    Args:
        ...
        temp_output_file (str): Path to temporary output file.
    ...
    with open(temp_output_file, "rb") as data:
        try:
            ...
            raise Exception(f"Error occurred while uploading to Azure: {e}") from e
...
def repopulate_cdn(blob_name):
    ...
    try:
        ...
        raise Exception(f"Failed to download blob {blob_name}. Error: {str(e)}") from e
...
def main(upload_path, input_file):
    ...
    try:
        upload_to_azure(f"{subfolder}/{blob_name}", temp_output_file)

...
complete -c impactanalyzer -s c -l commit -d "Git reference to compare against (default: HEAD)" -f
complete -c impactanalyzer -s d -l diff -d "show detailed diffs for the specified repository" -f
complete -c impactanalyzer -l diff-repo -d "specify which repository to show diffs for (optional with --diff)" -f
